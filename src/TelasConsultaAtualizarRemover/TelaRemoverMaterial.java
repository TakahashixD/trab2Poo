...
public class TelaRemoverMaterial extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TelaRemoverMaterial frame = new TelaRemoverMaterial();
	...
	public TelaRemoverMaterial() {
		...
		JLabel lblNewLabel = new JLabel("Escreva o ID do material que deseja remover:");
	...
	public void removerMaterial(int Id) {
		try {
			...
			st = conn.prepareStatement("DELETE FROM materiais WHERE ID = ?");
			st.setInt(1, Id);
